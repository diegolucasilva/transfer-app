...
internal class MoneyTransferUseCaseTest{
    ...
    private val transferMoneyUseCase = MoneyTransferUseCase(getAccountByNumberPort, transactionTransferMoneyPort)
    ...
    fun `When use case is executed should transfer money from an account to another account`() {
        ...
        val moneyTransfer = MoneyTransfer(fromAccount.number,toAccount.number,100.00)
    ...
    fun `When origin account has no funds should throws exception`() {
        ...
        assertEquals( "account ${fromAccount.number} does not have funds sufficient",thrown.messageError)
        verify(getAccountByNumberPort, times(1)).getByNumber(fromAccount.number)
        verify(getAccountByNumberPort, times(1)).getByNumber(toAccount.number)
    ...
    private fun whenUseCaseIsExecuted(moneyTransfer: MoneyTransfer){
        return transferMoneyUseCase.execute(moneyTransfer)
    }
